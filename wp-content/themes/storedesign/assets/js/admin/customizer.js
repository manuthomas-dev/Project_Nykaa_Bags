...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.storedesign webkitAnimationEnd.storedesign', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.storedesign webkitAnimationEnd.storedesign' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.storedesign webkitAnimationEnd.storedesign', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.storedesign webkitAnimationEnd.storedesign' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.storedesign', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.storedesign' );
			} );
		},
